...
const getSummaryTexts = (summary: StudentEnrollmentSummaryData): string[] => [
  summary.student,
  summary.guardians,
  summary.contactsAddress,
  summary.medicalInfo,
  summary.academic,
  summary.review,
];
    ...
    >
        ...
        <AppStack direction="row" spacing={1} sx={{ alignItems: 'center', mt: 1.25 }}>
          ...
          <AppBox sx={{ minWidth: 0 }}>
            ...
            <AppText sx={{ fontSize: fontSizes.xs, color: 'text.secondary', lineHeight: 1.3 }}>
              Acompanhe o progresso da matrícula
            </AppText>
          </AppBox>
        </AppStack>
          ...
          {stepLabels.map((label, index) => {
            ...
            return (
              <AppPaper
                ...
                variant="outlined"
                onClick={() => {
                  if (isCompleted) onStepSelect(index);
                }}
                sx={(theme) => ({
                  p: 1,
                  borderRadius: 1,
      ...
      >
        ...
        <AppText sx={{ fontSize: fontSizes.sm, lineHeight: 1.45, color: 'text.secondary' }}>
          Ao finalizar, a matrícula será criada conforme o contrato da API do backend.
        </AppText>
      </AppPaper>
    </AppStack>
  );
