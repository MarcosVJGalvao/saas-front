...
} from '@features/platform/clients/hooks/useClientOnboardingForm';
...
interface ClientOnboardingPageViewModel {
  ...
  form: Pick<
    ...
    'value' | 'uiExtras' | 'actions' | 'addressLoading' | 'planOptions' | 'plansLoading'
  >;
  handleNext: () => void;
  onCancel: () => void;
  onBack: () => void;
...
export const useClientOnboardingPageViewModel = (): ClientOnboardingPageViewModel => {
  ...
  return {
    ...
    maxCompletedStep,
    loading,
    form: { value, uiExtras, actions, addressLoading, planOptions, plansLoading },
    handleNext,
